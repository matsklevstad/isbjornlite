...
export const BeerSpawner = ({
  ...
  const normalVelocity: [number, number, number] = useMemo(
    () => [0, -15 - 20 * Math.random(), Math.random() * 20],
    []
  );
  const noGravityVelocity: [number, number, number] = useMemo(
    () => [0, Math.random() * -3, 0],
    []
  );
    ...
    const id = setInterval(() => {
      setBeers((prevBeers) => [
        ...
        <BeerModel
          ...
          position={[
            (Math.random() * 10 - Math.random() * 10) * scale,
            50,
            -40,
          ]}
          rotation={[
